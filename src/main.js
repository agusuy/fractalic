...
var IM_MAX_f = -0.6419194493770168, IM_MIN_f = -0.6419194502955501, RE_MIN_f = -0.2836084221809173,
...
var trabajo = function (){
	IM_MAX = IM_MAX_Array[index];
	IM_MIN = IM_MIN_Array[index];
	RE_MAX = RE_MAX_Array[index];
	RE_MIN = RE_MIN_Array[index];
	...
	server.scheduleAll(jobs, 2000, function (scheduled) {
		...
		return bitmap.writeFile("logs/video/result_"+index+".png", { type: imagejs.ImageType.PNG }).then(function () {
			// index=index+300;
			index++;
